...
const Navbar = () => {
    ...
    return (
        ...
        <div className="fixed z-[999] navbar w-full px-20 py-8 flex justify-between items-center">
            <div className='logo'>
            ...
            <div className='links flex gap-[4vw] items-center'>
                {["Services", "Our Work", "About Us", "Insights"].map((item , index)=>(
                    
                    <a key={index} className={`text-lg capitalize font-light ${index === 3 && "mr-[10vw]"}`}>{item}</a>
                    
                ))}
                {["Login", "Sign up"].map((item, index) => (
                    <button key={index} className={`text-lg capitalize ${index===1 && "-ml-4"}  ${index===1 && "bg-zinc-500"} py-2 px-4 rounded-full border-[1px] border-zinc-500 font-light`}>
                        <Link to= {`${index==0 ? "/login" : "/register"}`}>{item} </Link>
                    </button>
                ))}
            </div>
        </div>
